...
@router.get("/pick-rates/agents/team/{team_id}", response_model=AgentPickRateBase)
async def get_all_agents_pick_rates_from_team(team_id: int, db: AsyncSession = Depends(get_db)):
    team_result = await db.execute(select(Teams.team).where(Teams.team_id == team_id))
    team = team_result.scalars().first()
    ...
    agents = agents_result.all()
    agents = {record[0].agent_id: record[0].agent for record in agents if record[0].agent}
    total_result = await db.execute(select(func.sum(TeamsPickAgents.total_maps_played)).where(TeamsPickAgents.stage_id.in_(all_stages_ids)))    
